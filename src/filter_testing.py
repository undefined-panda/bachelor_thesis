...
def plot_graph(epochs, noise_level, def_acc_hist, cust_acc_hist, lr, bs, title=""):
    """
    Plots the accuracy-epochs graph for default and custom filters.
    """
...
def get_gaussian_filter(k=3, mu=0, sigma=1, normalize=True):
    """
    Creates the gaussian filter for the Canny-Algorithm.
    """
...
def test_canny(data, labels, noise, bs, epochs, lr, custom_filter_list, test_split=0.2, plot=True, title="", return_acc=False):
    ...
    canny_kernel = np.array([[get_gaussian_filter()]], dtype=np.float32)
